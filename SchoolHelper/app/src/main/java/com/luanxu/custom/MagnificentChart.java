...
// #MARK - Constructors
    ...
    public MagnificentChart(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            int backgroundColor = typedArray.getColor(R.styleable.MagnificentChart_backgroundColor, Color.parseColor("#FFFFFF"));
...
// #MARK - Drawing Methods
    ...
    private void animatedDraw(Canvas canvas){
        ...
        if(chartItemsList != null && maxValue >= 0){
            ...
            if(!isRound){
                if(isShadowShowing){
                    canvas.drawCircle(width/2, height/2, width/3, insideShadowPaint);
                }
                canvas.drawCircle(width/2, height/2, width/3, insideChartPaint);
            }
    ...
    private void drawInsideCircle(Canvas canvas, Paint insideShadowPaint, Paint insideChartPaint){
        if(!isRound){
    		...
    		paintArc.setTextSize(dp2px(20));
    		int textWidth=(int) paintArc.measureText(namber);
    		canvas.drawText(namber, width/2-textWidth/2   , width/2   , paintArc);
            
            paintArc.setColor(Color.GRAY);
            paintArc.setTextSize(dp2px(16));
            textWidth=(int) paintArc.measureText("学分");
    		canvas.drawText("学分",   width/2-textWidth/2 ,  width/2+dp2px(14)   , paintArc);
        }
    }
